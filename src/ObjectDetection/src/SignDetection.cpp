...
void DetectBlueArea(Mat full_sign, bool VERBOSE)
{
    ...
    Rect myROI1(0, 0, full_sign.size().width / 2, full_sign.size().height * 3 / 4);
    ...
    Rect myROI2(right_sign_width, 0, full_sign.size().width - right_sign_width, full_sign.size().height);
    Mat left_sign = image2(myROI2);

    //Top part of the image
    Mat image3(full_sign);
    Rect myROI3(full_sign.size().width / 2, 0, full_sign.size().width / 10, full_sign.size().height / 2);
    ...
    if (WhiteInLeft > WhiteInRight && WhiteInLeft - WhiteInRight > 100 && WhiteInLeft >= WhiteInTop && WhiteInTop == 0)
    {
        countRight++;
    }
    else if (WhiteInRight > WhiteInLeft && WhiteInRight - WhiteInLeft > 100 && WhiteInRight >= WhiteInTop && WhiteInTop == 0)
    {
        countleft++;
    }
...
array<bool, 3> ShapeDetection(Mat img, bool VERBOSE)
{
    ...
    for (vector<Point>::size_type i = 0; i < contours2.size(); i++)
    {
        ...
        if (approx2.size() == 4)
        {
            ...
            Mat image1(full_sign);
            Rect myROI1((full_sign.size().width) / 10, (full_sign.size().height) / 10, full_sign.size().width * 8 / 10, full_sign.size().height * 9 / 10);
